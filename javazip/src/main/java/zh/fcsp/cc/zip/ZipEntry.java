...
public class ZipEntry extends java.util.zip.ZipEntry implements Cloneable
{
	...
	{
		super
			.setExtra(ExtraFieldUtils.mergeLocalFileDataData(getExtraFields()));
	}
	...
	private static void performSetCompressedSize(ZipEntry ze, long size)
	{
		...
		catch (InvocationTargetException ite)
		{
			...
			throw new RuntimeException("Exception setting the compressed size "
				+ "of " + ze + ": " + nested.getMessage());
		}
		catch (Throwable other)
		{
			throw new RuntimeException("Exception setting the compressed size "
				+ "of " + ze + ": " + other.getMessage());
		}
	}
	...
	private static void checkSCS()
	{
		if (!triedToGetMethod)
		{
			synchronized (lockReflection)
			{
				...
				try
				{
					setCompressedSizeMethod = java.util.zip.ZipEntry.class
						.getMethod("setCompressedSize", new Class[]{Long.TYPE});
				}
